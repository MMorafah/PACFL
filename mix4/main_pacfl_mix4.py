...
for idx in range(args.num_users):
    ...
    local_ds = np.stack([train_ds_local[i][0].numpy() for i in range(len(train_ds_local))])
    length_ds = len(local_ds)
    local_ds = local_ds.reshape(length_ds, -1)
    local_ds = local_ds.T
     
    u_temp, sh_temp, vh_temp = np.linalg.svd(local_ds, full_matrices=False)
    u_temp=u_temp/np.linalg.norm(u_temp, ord=2, axis=0)
        
    U_clients.append(copy.deepcopy(u_temp[:, 0:K]))
...
#             best_glob_acc[k] = acc 
    ...
    if print_flag:
        ...
        for l in range(len(clusters)):
            ...
            if acc_all[argmax] > best_glob_acc[l]:
                best_glob_acc[l] = acc_all[argmax]
                
            avg_acc_glob.append(acc_all[argmax])
            print(f'Cluster {l}, USPS {acc_usps:.2f}, CIFAR-10 {acc_cifar10:.2f}, SVHN {acc_svhn:.2f}, FMNIST {acc_fmnist:.2f}')
            print(f'Cluster {l}, selectes {datasets_order[argmax]}, acc {acc_all[argmax]:.2f}')
            
        print(f'Avg Acc Clusters {np.mean(avg_acc_glob)}')
