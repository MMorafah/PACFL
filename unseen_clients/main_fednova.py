...
#         final_local_tloss.append(loss)           
    ...
    for key in updated_model:
        updated_model[key] = updated_model[key].to(args.device)
        if updated_model[key].type() == 'torch.LongTensor':
            updated_model[key] -= (coeff * d_total_round[key]).type(torch.LongTensor).to(args.device)
        elif updated_model[key].type() == 'torch.cuda.LongTensor':
            updated_model[key] -= (coeff * d_total_round[key]).type(torch.cuda.LongTensor).to(args.device)
        else:
...
template = ("Avg current_acc {:3.3f}, Avg best_acc {:3.3f}")
